...
void UITask::notify(UIEventType t) {
  ...
  if ((_node_prefs->buzzer_profile & 0x10)
      && _interfaceManager && _interfaceManager->isConnected()) return;
  switch(t){
...
void UITask::handleButtonLongPress() {
  ...
  if (millis() - ui_started_at < 8000) {   // long press in first 8 seconds since startup -> CLI/rescue
    ...
    // DL9SAU 2026-07-12: Button-Shutdown-Sperre. requestButtonShutdown()
    // prueft die Pref und meldet bei Sperre via Serial-Log + $companion.
    // Nur DIESE Button-Stelle ist gated -- CLI/Companion-'shutdown', USB-
    // Loss- und Batt-Low-Shutdown laufen unveraendert.
    if (the_mesh.requestButtonShutdown()) {
      shutdown();
    }
  }
}